...
@app.post('/atualiza', tags=[atualiza_tag],
          responses={"200": AtualizaViewSchema, "400": ErrorSchema})
...
def get_cota(form: BuscaCotaSchema):
    """Busca dados quantitativos de uma lista de fundos em um intervalo de datas.
    ...
    if not fundos_cotas:
        ...
        logger.warning(f"Erro ao buscar as cotas pelos dados'{lista_fundos,data_inicial,data_final}', {error_msg}")
        return {"message": error_msg}, 404
    else:
        logger.debug(f"Cotas encontradas para os fundos: '{lista_fundos}'")
        return apresenta_lista_cotas(fundos_cotas), 200


@app.get('/topfundos', tags=[cota_tag],
         responses={"200": FundoListaViewSchema, "404": ErrorSchema})
def get_top_fundos(query: BuscaTopFundosSchema):
    """Faz a busca por uma lista de fundos a partir do cnpj do fundo ou do nome do fundo.
    ...
    if fundos_cotas:
        ...
        logger.debug(f"Top fundos encontrados: '{[f.cnpj for f in fundos_cotas]}'")
        return apresenta_lista_fundos(fundos_cotas), 200
    else:
        error_msg = "Fundos não encontrados para os parâmetros solicitados :/"
        logger.warning(f"Erro ao buscar os fundo pelos dados'{classe,data_inicial,data_final}', {error_msg}")
        return {"message": error_msg}, 400
        


@app.post('/portfolio', tags=[portfolio_tag],
          responses={"200": PortfolioFundosViewSchema, "400": ErrorSchema})
def get_portfolio(form: BuscaPortfolioSchema):
    """Faz a busca da composição da carteira de uma lista de fundos para uma data de referência
    ...
    try:
        ...
        error_msg = "Erro na busca de dados do portfolio."
        logger.warning(f"Erro na busca de dados do portfolio para os parâmetros '{lista_datas,data_referencia}', {e}")
        return {"message": error_msg,"description":str(e)}, 400
